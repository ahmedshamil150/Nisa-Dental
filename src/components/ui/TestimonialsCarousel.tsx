...
export function TestimonialsCarousel({ testimonials }: { testimonials: any[] }) {
  ...
  const items = (Array.isArray(testimonials) ? testimonials : []).filter((t) => t && typeof t === "object")
  const t = items[index]
      ...
      <div className="max-w-container-max mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-end mb-10 md:mb-16 gap-6">
          <div className="max-w-md">
            ...
            <p className="text-on-surface-variant">Don&apos;t just take our word for it&mdash;hear from the thousands of patients who have trusted us with their smiles.</p>
          </div>
        </div>
          ...
          {items.map((t: any) => (
            <div key={t.id} className="bg-surface p-8 rounded-xl border border-outline-variant/30 shadow-sm relative group hover:-translate-y-2 transition-transform duration-300">
              <div className="flex text-primary mb-4">
                {Array.from({ length: t?.rating && t.rating > 0 ? t.rating : 0 }).map((_, i) => (
              ...
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-full bg-secondary-container flex items-center justify-center font-bold text-primary">
                  {((t.patient_name ?? "Anonymous") ? (t.patient_name ?? "Anonymous").charAt(0) : "?")}
                </div>
                <div>
                  <div className="font-label-md text-label-md text-on-surface">{(t.patient_name ?? "Anonymous")}</div>
